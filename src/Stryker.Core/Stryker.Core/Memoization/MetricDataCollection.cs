...
public class MetricDataCollection : IMetricDataCollection
{
    ...
    public void Add(MetricData entry)
    {
        ...
        if (!_orderedMetricData.TryGetValue(entry.Identifier, out var metricDatas))
        {
            _orderedMetricData[entry.Identifier] = new ConcurrentBag<MetricData>();
        }
    ...
    public MetricResultReport CreateMetricResultReport()
    {
        ...
        return new MetricResultReport
        {
            ...
            TotalUniqueMemoizationInjectionCalls = _orderedMetricData.Count,
            TotalHits = totalHits,
